...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if args_cli.num_envs not in (None, 1):
        print(f"[DQN] overriding num_envs={args_cli.num_envs} to 1 for single-env training.")
    ...
    while simulation_app.is_running():
        for episode in tqdm(range(n_episodes)):
            episode_return, timestep, mean_q_loss = agent.learn(env)
            ...
            writer.add_scalar("loss/q_loss", mean_q_loss, episode)

            if episode % 50 == 0:
                print(
                    f"[{algorithm_name}] episode {episode} "
                    f"return={episode_return:.2f} steps={timestep} "
                    f"epsilon={agent.epsilon:.4f} q_loss={mean_q_loss:.4f}"
                )
